/**
 * Same DDL as schema.sql, exposed as a string so it can be served over
 * HTTP (GET /api/v1/db/schema) from both the Express dev server and the
 * Cloudflare Worker runtime, neither of which can read schema.sql off disk
 * at request time (esp. in Workers, which has no filesystem).
 *
 * Keep this in sync with schema.sql whenever the schema changes.
 */
export const SCHEMA_DDL = `-- NEXORUM OS — D1 (SQLite) schema
