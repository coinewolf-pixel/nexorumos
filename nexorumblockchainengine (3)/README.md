...
# 6. Деплой (сначала соберёт фронтенд npm run build, потом wrangler deploy)
npm run deploy
```

`wrangler.toml` уже в `.gitignore` — секреты и `database_id` никогда не попадут в git.

**Важно про статику.** `src/worker.ts` отдаёт собранный React-фронтенд (`dist/`) через биндинг `[assets]` в `wrangler.toml` — это уже настроено в `wrangler.toml.example`, ничего добавлять не нужно. Но если открыть задеплоенный сайт и увидеть пустую страницу/500-ошибку вместо приложения — почти всегда это одна из двух причин:

- Деплой запускался напрямую как `wrangler deploy`, минуя `npm run deploy` (или `npm run build` не выполнялся до этого) — тогда `dist/` пустой или устаревший. Всегда деплойте через `npm run deploy`.
- В своём `wrangler.toml` нет блока `[assets]` (например, если конфиг создавался вручную, а не копированием `wrangler.toml.example`) — тогда воркер отвечает только на `/api/*`, а сам сайт не раздаётся.

Если `wrangler deploy` падает с ошибкой ещё до открытия сайта — это обычно `npx wrangler login` (не авторизован) или неверный/пустой `database_id` в `wrangler.toml`.

### Деплой через Git-интеграцию (Cloudflare Dashboard → Workers Builds)

Если проект подключён к Cloudflare как Git-репозиторий (автодеплой при пуше, а не ручной `wrangler deploy` с компьютера) — сборка настраивается отдельно, в **Cloudflare Dashboard → Workers & Pages → ваш проект → Settings → Build**, и команды из package.json там не подхватываются автоматически. По умолчанию Cloudflare ставит **Deploy command = `npx wrangler deploy`**, который сам по себе фронтенд не собирает — из-за этого будет ошибка `Could not detect a directory containing static files`, потому что `dist/` ещё не существует.

Исправление — в настройках Build задать:

- **Build command:** `npm run build`
- **Deploy command:** `npx wrangler deploy` (можно оставить по умолчанию)

Либо проще — одной строкой:

- **Deploy command:** `npm run deploy`

Также `database_id` в `wrangler.toml` из репозитория должен быть настоящим (не плейсхолдером) — либо задайте его через переменные окружения проекта в дашборде.
